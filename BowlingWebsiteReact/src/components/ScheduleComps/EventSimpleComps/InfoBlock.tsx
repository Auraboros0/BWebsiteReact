...
function InfoBlock(props: tournament) {
    ...
    if (props.gender == 2) {
        return (
            <div className='infoBlock' style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                {sameCenter ?
                    <>
                        <h2>{props.center[0]}</h2>
                        <h2>{props.city[0].street},</h2>
                        <h2>{props.city[0].city}, {props.city[0].state},</h2>
                        <h2>{props.city[0].zip}</h2>
                    </>
                    :
                    <div style={{ display: 'flex', gap: '20px'}}>
                        <div>
                            <h2>M @ {props.center[0]}</h2>
                            <h2>{props.city[0].street},</h2>
                            <h2>{props.city[0].city}, {props.city[0].state},</h2>
                            <h2>{props.city[0].zip}</h2>
                        </div>
                        <div>
                            <h2>W @ {props.center[1]}</h2>
                            <h2>{props.city[1].street},</h2>
                            <h2>{props.city[1].city}, {props.city[1].state},</h2>
                            <h2>{props.city[1].zip}</h2>
                        </div>
                    </div>
                }

                {/* {isMd && <h3 style={{ position: 'absolute', bottom: '0' }}>{startMonth}/{startDay} - {endMonth}/{endDay}</h3>} */}
            </div>
        )
    } else {
        return (
